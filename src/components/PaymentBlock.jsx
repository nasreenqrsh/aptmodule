...
const PaymentBlock = ({ totalAmount = 0, prefillPaymentData }) => {
  ...
  const change = amount && !isNaN(amount) ? Math.max(0, parsedTotalAmount - totalPaid - parseFloat(amount)) : parsedTotalAmount - totalPaid;
    ...
    <div className="pymntblock">
        ...
        {payments.length > 0 && (
          <>
            ...
            <div className="frmdiv totalpaidrow">
              <strong>Total Paid:</strong> {totalPaid.toFixed(2)} / {parsedTotalAmount.toFixed(2)}
            </div>
          </>
        )}
      </div>
      ...
      {payments.length > 0 && (
        <div className="frmdiv" style={{ textAlign: 'center', marginTop: '20px' }}>
          <button className="submitbtn" onClick={handleSubmitPayments} disabled={!isCompleteEnabled}>
            Complete Invoice
          </button>
        </div>
      )}
