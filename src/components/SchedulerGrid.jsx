...
const AppointmentScheduler = () => {
  ...
  const renderAppointments = (time, doctor) => {
    ...
    const filteredAppointments = appointments.filter((appt) => {
      ...
      const doctorMatch = normalizeDoctorName(appt.doctorname) === normalizeDoctorName(doctor);
    ...
    return filteredAppointments.map((appt, idx) => {
      ...
      return (
        <div
          ...
          style={{ width: `${width}px`, minWidth: '50px', boxSizing: 'border-box' }}
        >
          <div className="ptflx">
            <div className="ptnm">{appt.fullname}</div>
            <div className={`aptst ${statusClass}`}><span></span>{appt.status || 'Booked'}</div>
          </div>
          <div className="apptype"><strong>{appt?.servicename || appt?.servicecode || 'N/A'}</strong></div>
          <span
            className="expopup"
            onClick={() => {
        ...
        <AppointmentDetails
          ...
          onEdit={(appt) => {
            setEditData({
              ...
              service: {
                servicename: appt.servicecode || appt.servicename,
                practitioner: appt.doctorname,
                start: appt.starttime,
                end: appt.endtime,
                room: appt.room,
                note: appt.notes,
                duration: appt.duration?.replace('mins', '').trim() || '5',
              }
            });
